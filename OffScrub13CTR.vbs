'*******************************************************************************
' Name: OffScrub_O15c2r.vbs
' Author: Microsoft Customer Support Services
' Copyright (c) 2012 Microsoft Corporation
' Script to remove Office 2013 Click To Run (C2R) products
...
Const SCRIPTVERSION = "1.86"
Const SCRIPTFILE    = "OffScrub_O15c2r.vbs"
Const SCRIPTNAME    = "OffScrub_O15c2r"
Const RETVALFILE    = "ScrubRetValFile.txt"
Const OVERSION      = "15.0"
Const OVERSIONMAJOR = "15"
Const OREGREF       = "OFFICE15."
Const OREGREFC2R    = "Microsoft Office 15"
Const ONAME         = "Office 2013 C2R"
...
Const ERROR_INCOMPLETE              = 32  'Bit #6.  Removal needs to run again after a system reboot.
...
Dim sRetVal, sScriptDir, sPackageGuid
...
Dim fIsElevated, fNoElevate, fUserConsent
...
sTmp = "Stage # 0 " & chr(34) & "Basic detection" & chr(34) & " (" & Time & ")"
...
If dicC2RSuite.Count > 0 Then 
    Log "Registered product(s) found:"
...
'---------------------------------------------------
RegReadValue HKLM, "SOFTWARE\Microsoft\Office\15.0\ClickToRun", "PackageFolder", sPackageFolder, "REG_SZ"
' if sPackageFolder is invalid set it to the c2r registry reference string
If NOT Len(sPackageFolder) > 0 OR IsNull(sPackageFolder) Then sPackageFolder = OREGREFC2R

sPackageGuid = ""
RegReadValue HKLM, "SOFTWARE\Microsoft\Office\15.0\ClickToRun", "PackageGUID", sPackageGuid, "REG_SZ"
...
sTmp = "Stage # 1 " & chr(34) & "Uninstall" & chr(34) & " (" & Time & ")"
Log vbCrLf & sTmp & vbCrLf & String(Len(sTmp), "=") & vbCrLf

' clean O15 SPP
'--------------
Log vbCrLf & "Clean OSPP" & " (" & Time & ")"
CleanO15SPP

' end all running Office applications
'------------------------------------
Log vbCrLf & "End running processes" & " (" & Time & ")"
CloseOfficeApps
...
' unpin shortcuts
'----------------
Log vbCrLf & "Unpinning shortcuts" & " (" & Time & ")"
CleanShortcuts sAllUsersProfile, False, True
CleanShortcuts sProfilesDirectory, False, True

' uninstall
'----------
Log vbCrLF & "Removing " & ONAME  & " (" & Time & ")"
Uninstall

                                '---------------------
                                ' Stage # 2 - CleanUp |
                                '---------------------
sTmp = "Stage # 2 " & chr(34) & "CleanUp" & chr(34) & " (" & Time & ")"
Log vbCrLf & sTmp & vbCrLf & String(Len(sTmp), "=") & vbCrLf
...
If fTestRerun Then
    Log vbCrLf & "Enforcing 'Rerun' mode for test purposes"
...
Sub ExitScript
    ...
    If CBool(iError AND ERROR_INCOMPLETE) Then 
        Log vbCrLf & "Removal result: " & iError & " - INCOMPLETE. Uninstall requires a system reboot to complete."
    Else
        sTmp = " - SUCCESS"
        If CBool(iError AND ERROR_USERCANCEL) Then sTmp = " - USER CANCELED"
        If CBool(iError AND ERROR_FAIL) Then sTmp = " - FAIL"
        Log vbCrLf & "Removal result: " & iError & sTmp
    End If
    ...
    Log vbCrLf & "Removal end: " & vbTab & Time
...
Sub ShowSyntax
    Wscript.Echo sErr & vbCrLf & _
             ...
             "Usage:" & vbTab & SCRIPTFILE & " C2R" & vbCrLf & vbCrLf & _
...
Sub FindInstalledOProducts
    Dim ArpItem
    Dim sCurKey, sValue, sConfigName, sCulture, sDisplayVersion, sUninstallString
    Dim iLeft, iRight
    Dim arrKeys
    ...
    Log "Detecting installed products " & " (" & Time & ")"
    
    ' identify C2R products
    If RegEnumKey(HKLM, REG_ARP, arrKeys) Then
        ' configuration products
        ' pre-filter on C2R and .MSI Office products
        For Each ArpItem in arrKeys
            ...
            If (fSystemComponent0 AND fDisplayVersion AND InStr(UCase(sUninstallString), UCase(OREGREFC2R)) > 0) Then
                ...
                If NOT dicInstalledSku.Exists(ArpItem) Then dicInstalledSku.Add ArpItem, sConfigName & " - " & sDisplayVersion 
                ' categorize the SKU as C2R
                If NOT dicC2RSuite.Exists(ArpItem) Then dicC2RSuite.Add ArpItem, sConfigName & " - " & sDisplayVersion 
            ElseIf (fSystemComponent0 AND fDisplayVersion AND InStr(UCase(ArpItem), UCase(OREGREF)) > 0) Then
            ...
            If InScope(ArpItem) Then
                ...
                Case Else
                    ...
                    If NOT dicKeepSku.Exists(ArpItem) Then dicKeepSku.Add ArpItem, ArpItem
                End Select
            End If 'InScope
        Next 'ArpItem
    End If 'RegEnumKey
...
Sub CleanO15SPP
    ...
    sCleanO15SPP = "CleanO15SPPx86.exe"
    If f64 Then sCleanO15SPP = "CleanO15SPPx64.exe"
    If oFso.FileExists(sScriptDir & sCleanO15SPP) Then
        sCmd = sScriptDir & sCleanO15SPP
        Log "   Running: " & sCmd
        On Error Resume Next
...
Sub DelSchtasks ()
    ...
    Log "Removing scheduled tasks" & " (" & Time & ")"
        ...
        For Each Process in Processes
            For Each prop in Process.Properties_
                If prop.Name = "ExecutablePath" Then 
                    If InStr(UCase(prop.Value), UCase(OREGREFC2R)) > 0 Then
                        If NOT InStr(sUserWarn, Process.Name) > 0 Then sUserWarn = sUserWarn & vbCrLf & " - " & Process.Name
                    End If 
...
Sub Uninstall
    ...
    Log " - Checking state of OSE service" & " (" & Time & ")"
    ...
    Log " - Removing published component registration for C2R packages" & " (" & Time & ")"
    ' delete the manifest files
    If oFso.FolderExists(sPackageFolder & "\root\Integration") Then
        sCmd = "cmd.exe /c del " & chr(34) & sPackageFolder & "\root\Integration\C2RManifest*.xml" & chr(34)
        Log "   Running: " & sCmd
        sReturn = oWShell.Run (sCmd, 0, True)
        Log "   Return value: " & sReturn
        If oFso.FileExists(sPackageFolder & "\root\Integration\integrator.exe") Then
            ' Beta2
            sCmd = chr(34) & sPackageFolder & "\root\Integration\integrator.exe" & chr(34) & " /U  /Extension PackageRoot=" & chr(34) & sPackageFolder & "\root" & chr(34) & " PackageGUID=" & sPackageGuid
            Log "   Running: " & sCmd
            sReturn = oWShell.Run (sCmd, 0, True)
            Log "   Return value: " & sReturn
            ' Beta1
            sCmd = chr(34) & sPackageFolder & "\root\Integration\integrator.exe" & chr(34) & " /U"
            Log "   Running: " & sCmd
            sReturn = oWShell.Run (sCmd, 0, True)
            Log "   Return value: " & sReturn
        End If
        If oFso.FileExists(sProgramData & "\Microsoft\ClickToRun\{" & sPackageGuid & "}\integrator.exe") Then
            sCmd = chr(34) & sProgramData & "\Microsoft\ClickToRun\{" & sPackageGuid & "}\integrator.exe" & chr(34) & " /U  /Extension PackageRoot=" & chr(34) & sPackageFolder & "\root" & chr(34) & " PackageGUID=" & sPackageGuid
            Log "   Running: " & sCmd
            sReturn = oWShell.Run (sCmd, 0, True)
            Log "   Return value: " & sReturn
        End If
    ...
    Log " - Removing C2R and App-V registry data" & " (" & Time & ")"
    If fC2R Then
        ...
        Do
            If RegEnumValues(hDefKey, sSubKeyName, arrNames, arrTypes) Then
                For Each Name in arrNames
                    If InStr(LCase(Name), LCase(OREGREFC2R)) > 0 Then RegDeleteValue hDefKey, sSubKeyName, Name, False
                Next 'Name
    ...
    Log " - Detect Msi based products" & " (" & Time & ")"
    For Each prod in oMsi.Products
        If CheckDelete(prod) Then
            Log vbCrLf & "Calling msiexec.exe to remove " & prod & " (" & Time & ")"
            ...
            Log " - msiexec returned: " & SetupRetVal(sReturn) & " (" & sReturn & ")" & " (" & Time & ")"
...
Sub Regwipe
    ...
    If fC2R Then
        ...
        ' Office key in HKLM
        ClearOfficeHKLM "SOFTWARE\Microsoft\Office\" & OVERSION
    End If

    If fC2R OR fRemoveAll Then
        ' Run key
        ...
        If RegEnumValues (HKLM, sSubKeyName, arrNames, arrTypes) Then
            For Each name in arrNames
                If RegReadValue(HKLM, sSubKeyName, name, sValue, "REG_SZ") Then
                    If InStr(LCase(sValue), LCase(OREGREFC2R)) > 0 Then RegDeleteValue HKLM, sSubKeyName, name, False
                End If
            Next 'item
        End If
    ...
    Log " - Scanning Windows Installer metadata for removeable UpgradeCodes" & " (" & Time & ")"
    For iLoopCnt = 1 to 5
    ...
    Log " - Scanning Windows Installer Global Components metadata" & " (" & Time & ")"
    ...
    Log " - Scanning Windows Installer Published Components metadata" & " (" & Time & ")"
...
Sub FileWipe
    ...
    Log vbCrLf & "File Cleanup" & " (" & Time & ")"

    CloseOfficeApps
    DelSchtasks

    ' remove the OfficeSvc service
    Log " - Deleting OfficeSvc service" & " (" & Time & ")"
    DeleteService "OfficeSvc"

    ' SP1 addition / change
    ' remove the ClickToRunSvc service
    Log " - Deleting ClickToRunSvc service" & " (" & Time & ")"
    ...
    If fC2R Then
        Log " - Deleting C2R package files" & " (" & Time & ")"
        If oFso.FolderExists(sProgramFiles & "\Microsoft Office 15") Then
            Log "   Attention: Now closing Explorer.exe for file delete operations"
            Log "   Explorer will automatically restart."
            wscript.sleep 2000
            CloseOfficeApps
        End If
        Log " - Deleting Office folders" & " (" & Time & ")"
        DeleteFolder sProgramFiles & "\Microsoft Office 15"
        If f64 Then DeleteFolder sCommonProgramFilesX86 & "\Microsoft Office 15"
    ...
    ' delete shortcuts
    Log " - Search and delete shortcuts" & " (" & Time & ")"
    ...
    Log "File Cleanup complete" & " (" & Time & ")"
End Sub ' FileWipe
...
Sub CleanShortcuts (sFolder, fDelete, fUnPin)
	...
	For Each file In oFolder.Files
		If LCase(Right(file.Path, 4)) = ".lnk" Then
            ...
            ' C2R shortcuts
            If InStr(sc.TargetPath, sPackageFolder) > 0 Then
...
Sub UnPin(file)
    ...
    For Each verb in fldItem.Verbs
        ...
        Case "Unpin from Taskbar", "Von Taskleiste l?sen", "D?tacher du barre des t?ches", "D?tacher de la barre des t?ches", "Desanclar de la barra de tareas", "Ta bort fr?n Aktivitetsf?ltet", "??? ????????(K)", "?? ????? ??(K)", "????????? ?? ?????? ?????"
            verb.DoIt
        Case "Unpin from Start Menu", "Vom Startmen? l?sen", "D?tacher du menu D?marrer", "D?tacher de la menu D?marrer"
...
'   GetMyProcessId
'
'   Returns the process id of self
'-------------------------------------------------------------------------------
Function GetMyProcessId()
    Dim iParentProcessId

    iParentProcessId = 0
    ' try to obtain id from a new child cscript instance
    ' error handled by fallback to use of Win32_Process
    On Error Resume Next
    iParentProcessId = GetObject("winmgmts:root\cimv2").Get("Win32_Process.Handle='" & oWShell.Exec("cscript.exe").ProcessId & "'").ParentProcessId
    On Error Goto 0
    If iParentProcessId > 0 Then
...
Function CheckDelete(sProductCode)
    ...
    If dicKeepProd.Exists(UCase(sProductCode)) OR dicKeepSku.Exists(UCase(sProductCode)) Then Exit Function
...
Sub ScheduleDeleteEx ()
    ...
    Log " - Adding " & dicDelInUse.Count & " PendingFileRenameOperations" & " (" & Time & ")"
...
Sub Rerun ()
    ...
    If RegReadValue(HKCU, "SOFTWARE\Microsoft\Office\15.0\CleanC2R", "Rerun", sValue, "REG_DWORD") Then
        ...
        Log vbCrLf & "Error: Removal failed"
